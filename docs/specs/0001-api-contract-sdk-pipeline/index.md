...
| Orvano metadata | OpenAPI extensions set via TypeSpec `@extension`: `x-orvano-audience`, `x-orvano-service`, `x-orvano-event`, `x-orvano-idempotent`, `x-orvano-since`; `x-orvano-upload` is reserved and gets its templates with Buckets & files (row 20) | Carries what OpenAPI lacks (who may call it, grouping, events) without a custom format. |
...
```
A failing step can instead expect `{ status: 409, code: user_already_exists }`. SdkGen also emits a **test only dispatch table** per language (`operationId` → the generated method), so each SDK needs only one small scenario interpreter rather than one handwritten test per scenario. The dispatch table is excluded from published packages. `@orvano/console-client` gets its own dispatch table, which the JS interpreter loads next to the `@orvano/js` one to run `as: console` steps.

**Test fixtures**: the server supports a test only startup flag (`ORVANO_TEST_FIXTURES=<path>`, refused unless the environment is `Test`) that loads `tests/scenarios/fixtures.yaml` to seed known projects, users, and API keys. Its exact shape waits for the platform data model (row 3); the health scenario needs no fixtures.

**Response validation** (AC-9): in the `Test` environment a middleware reads the raw `openapi.json` (not the `Microsoft.OpenApi` object model), strips keywords only OpenAPI uses (`discriminator` mapping hints, `example`, `xml`, `externalDocs`), registers each `components/schemas` entry in a `JsonSchema.Net` schema registry under the document's base URI so `#/components/schemas/X` references resolve, then validates each response body against the operation's response schema for that status.
...
| Auth | session token or API key header, format decided in the auth spec (row 8); API key only accepted from server audiences |
| Console routes | `console` operations live under `/v1/console/*`; only a console session is accepted there (format decided with rows 7 and 8); project scoped ones still send `X-Orvano-Project` |
| Lists | query `cursor`, `limit`; response `{ items, nextCursor }`; `nextCursor` null on the last page |
| Errors | `application/problem+json` with `type`, `title`, `status`, `detail`, `code`, `requestId` |
...
| Server call | API key | server `Client` config (server audience only) |
| Client call | session token | session store; token format decided in spec for row 8 |
| Error thrown | status, code, message, requestId | Problem Details body (`code` and `requestId` are extension members) and `X-Request-Id` header |
...
- Console audience: a test only `console` operation appears in `@orvano/console-client` and in no public package; calling it with an API key returns 401, calling it with an app user session returns 401, and with a console session it succeeds (a `console` scenario); it is absent from `openapi.public.json`. Verifies **AC-17**.
...
6. Problem Details responses on the server, the error code catalog in the contract, and the SDK exception type. Satisfies **AC-6**.
7. Audience routing and the shared client pattern: pluggable auth providers, TS entry points, Dart package split, .NET inclusion, browser guard on the key setter. Satisfies **AC-4**.
8. The `console` audience: the private `@orvano/console-client` package, the `/v1/console` route group that accepts only a console session, a test only console operation, a test console session seeded through `ORVANO_TEST_FIXTURES` (until rows 7 and 8 define the real one), and `as: console` scenario support in the JS interpreter (with the console client's own dispatch table). Add the CI check that no published package depends on `@orvano/console-client`, and set `IsPackable=false` on `Orvano.Contract`. Satisfies **AC-2**, **AC-17**.
9. Cursor list convention and async iterators (tested with a small paged test operation). Satisfies **AC-7**.
10. Event schema convention and the generated event registry (decode only; the socket comes with realtime, row 22). Satisfies **AC-8**.
...
- [ ] The auth spec (row 8) decides the session token and API key header formats referenced in the API conventions.
- [ ] Claim the package names before the first publish. Checked 2026-09-24: nothing is published under the `@orvano` npm scope, and `orvano_core` (pub.dev) and `Orvano` (NuGet) are unused. Still to do: create the `orvano` npm org, a pub.dev verified publisher (needs a domain), and a NuGet organization with the `Orvano` prefix reserved.
- [ ] Agent Skills: the .NET, Aspire, and pnpm skills were installed with spec 0002 (`.claude/skills/`). No skill was found for TypeSpec; Scriban and Dart/Flutter were not searched yet. Consider searching for those before Milestone 1.
- [ ] Before Milestone 1, confirm the `Microsoft.OpenApi` 2.x release you pin reads OpenAPI 3.1 in a stable (not preview) version; if not, parse `openapi.json` with `System.Text.Json` into the internal model directly.
