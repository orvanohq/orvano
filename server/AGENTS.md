...
- Platform tables carry the module name as a prefix (`orvano.platform_projects`). Migrations are `NNNN_<name>.sql` and are checksummed when applied, so never edit one after it merges; add a new file instead.
